...
#  Section 7: Debugging and Utility Macros/Variables.
#
################################################################################
...
#                     In addition to the above values for 'display', the text "+target" (or +T) can also be appended, 
...
# LINK_DEPS  - Full recipe for linking all listed dependencies with the standard recipe and libraries
  LINK      = ${VERBOSE}${CXX}  -o $@    $(strip ${CXXFLAGS} ${LDFLAGS} ${??STATIC_FLAGS})
...
# CUDAFLAGS - flags for the CUDA compiler. These will be merged with CXXFLAGS using -Xcompiler.
  CUDAFLAGS = -D FLOAT -D SHORT -D _FORCE_INLINES -D _CUDA_CALC_ -D NDEBUG \
              ...
              -use_fast_math  -Wno-deprecated-gpu-targets
...
# It may also be useful to look at the commands below for reference.
  ...
  # If making Multifind, INCLUDESVM must indicate the location of svm.o from the libsvm package. 
...
######################### GPU/CUDA ############################
# The CUDA environment variable indicates that RNAstructure programs should
# use the CUDA versions of partition and fold.
# This requires building the "normal" classes with the CUDA compiler (nvcc) and
# CUDA flags as well as compiling the CUDA-specific versions of partition and Fold.
ifneq (${CUDA},)
  $(info CUDA environment variable detected: compiling for GPU)
  CXX=${CUDA_COMPILER} # use nvcc to compile "normal" RNAstructure code.
  DYNAMIC_FLAGS=# --as-needed flag is not supported by nvcc
  CXXFLAGS := $(CUDAFLAGS) $(patsubst %,-Xcompiler %,$(CXXFLAGS)) # pass the CUDA flags as well as the standard CXX compiler flags.
endif
...
APP_MAKE_VARS=COMPILE_CUDA COMPILE_DYNALIGN_II COMPILE_DYNALIGN_II_SMP COMPILE_INSTRUMENTED  COMPILE_MULTIFIND COMPILE_SMP COMPILE_SVM COMPILE_SVM_SMP CUDA_COMPILER CUDAFLAGS CXXFLAGS_OPENMP INCLUDE_GAMMA INCLUDESVM LINK_CUDA LINK_SMP
