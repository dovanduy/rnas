...
################################################################################
function runMake() {
    local silent=0   required=0  errors=0 force="$REMAKE_TEST_PROGS"
    targets=()
    for arg; do
        case "$arg" in 
            '') ;; # skip empty
...
################################################################################
# Verifies that each of its arguments is an existing file and shows a warning
# if it is missing otherwise. 
# If the -r flag is passed, then any missing file causes the script to exit
#   with a status of 1. Otherwise, the function returns 1, but exit is not called.
# Usage: verifyInputFiles [FILES...] [-r]
################################################################################
function verifyInputFiles() {
    local file files=() errors=0 action=errmsg # action to take if a file is missing: errmsg or die
    for arg; do
        case "$arg" in 
            '') ;; # skip empty
            -r|--required) action=die    ;;
             *) files+=("$arg")        ;; 
        esac
    done
...
function listOrphanedFiles() {
    filesToArray . -maxdepth 1 -type f  ! -name Makefile ! -name '!*'
