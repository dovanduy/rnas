...
int TurboFold_Interface::run() {
	...
	if (fasta_sequences != NULL)
		object = new TurboFold(fasta_sequences, &outputPfsFiles, OutAln);
	else
		object = new TurboFold(&sequenceFiles, &outputPfsFiles, OutAln);
	...
	if (is_RSample_mode) {
		...
		rsdata = new RsampleData(upFile.c_str(), peFile.c_str(), pmFile.c_str());
