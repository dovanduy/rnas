...
int RNA::FoldSingleStrand(const float percent, const int maximumstructures, const int window, const char savefile[], const int maxinternalloopsize, bool mfeonly, bool simple_iloops, bool disablecoax) {
    char *savefilename;
    int percenti;
    int tracebackstatus;
    ...
    tracebackstatus = dynamic(ct, data, maximumstructures, percenti, window, progress, false, savefilename, maxinternalloopsize, mfeonly, simple_iloops, disablecoax);
...
// by sampling from paired-mid/paired-end/unpaired shape reactivity distributions
  ...
  for ( i=1; i<= numsamples; i++) {
    // 1st nt
    if ( GetPair(1,i) ==0 ) { //u
      shape_predicted[1]+=rnd(ref_uu);
    }
    else { //p-end
      shape_predicted[1]+=rnd(ref_pe);
    }
    // nt's 2 to seq_length-1
    for(int j=2; j<=seq_length-1;j++) {
      if (GetPair(j,i) == 0 ) { //u
        shape_predicted[j]+=rnd(ref_uu);
      }
      else if ( GetPair(j-1,i) != 0 && GetPair(j+1,i) != 0 ) { //p-mid
        shape_predicted[j]+=rnd(ref_pm);
      }
      else { //p-end
        shape_predicted[j]+=rnd(ref_pe);
      }
    }
    //last nt
    if (  GetPair(seq_length,i) == 0 ) { //u
      shape_predicted[seq_length]+=rnd(ref_uu);
    }
    else { //p-end
      shape_predicted[seq_length]+=rnd(ref_pe);
    }
  }
...
int RNA::PartitionFunction(const char savefile[], double temperature, bool disablecoax, bool restoreSHAPE) {
    ...
    pfdata=new pfdatatable(data,scalingdefinition,(temperature<0?GetTemperature():temperature));
    ...
    calculatepfunction(ct,pfdata,progress,savefilename,false,&Q,w,v,wmb,wl,wmbl,wcoax,fce,w5,w3,mod,lfce,disablecoax);
...
//so RNA::GetPairProbability(i,j) will return V(i,j)*1/1, which will be the pairing probability
	...
	if (savefilename!=NULL) {
        if (!progress||!progress->canceled()) writepfsave(savefilename,ct,w5,w3,v,w,wmb,wl,wmbl,wcoax,fce,mod,lfce,pfdata);
...
int RNA::ReadDMS(const char filename[]) {
    int code = ct->ReadSHAPE(filename, RESTRAINT_DMS); //call ReadSHAPE() to read the file and determine pseudo energies
    if (ErrorCode==0) ErrorCode=code; // set the error code, but ONLY if it hasn't already been set (we don't want to hide a previous error)
    return code;    
}
...
double RNA::GetEnsembleEnergy() {
    ...
    return -RKC*GetTemperature()*(log(w5[ct->GetSequenceLength()])-ct->GetSequenceLength()*log(pfdata->scaling));
}

// Gets the ensemble defect.
double RNA::GetEnsembleDefect(int structurenumber) {
	//check to see if partition function data is present.
	if (!partitionfunctionallocated)
		PartitionFunction();
	...
	for(int i=1;i<=GetSequenceLength();++i){//for every nucleotide in the structure
		...
		if(GetPair(i, structurenumber)==0){
			...
			for(int j=1;j<=GetSequenceLength();++j){
				if(i<j)
					defect+=GetPairProbability(i,j);//add the probabilities to the 'defect'
				else if(j<i) 
					defect+=GetPairProbability(j,i);
			}
		} 
		else if(GetPair(i, structurenumber)>i)
			defect+=2*(1-GetPairProbability(i,GetPair(i, structurenumber)));
...
int RNA::GetPairProbabilities(double* arr, const int size){
    ...
    for(int i=0; i < len; i++)
        for(int j=i+1; j < len; j++)
            arr[pos++] = (double)calculateprobability(i+1,j+1,v,w5,ct,pfdata,lfce,mod,pfdata->scaling,fce);
    ...
    try {
        ...
        switch(type) {
            ...
            case FILE_PFS: { //partition function save file
                ...
                readpfsave(filename, ct, w5, w3,v, w, wmb,wl, wmbl, wcoax, fce,&pfdata->scaling,mod,lfce,pfdata,data);
                return 0;
            }  // FILE_PFS
            #ifndef PARTITION_CORE // exclude these from minimal partition-only builds (e.g. partition-rosetta)
