...
int structure::SetSequence(const string& sequence) {
	...
	allocate(count);
	count=0;
	// Setup the CT sequence (i.e. arrays nucs, numseq, numofbases)
...
double structure::CalculatePseudoEnergy(const double data, const RestraintType modifier, const double slope, const double intercept, const bool use_params_if_available ) {
	...
	// If we get here, we are using DMS, CMCT, SHAPE_AC or SHAPE_GU.
	// RMW: Why do slope and intercept matter here? They are not used in the actual energy calculation (i.e. in the Potential function).
	if( data < 0 || (slope == 0 && intercept == 0 && (!use_params_if_available || params->empty()) ))
		return 0;

	// RMW: What if data < 0, but shape or intercept is not 0. Should we return `intercept` or does `Potential` handle this case? (perhaps by returning NaN, which is handled below)
	const double val  = Potential(data, *params, kT);
	...
	while (!(in >> position >> data).fail()) { // this will ensure the last data point is read, even if there is no newline at the end of the file. (EOF can be set after a valid datapoint is read if there is no whitespace following it.)
		...
		if (position<1||position>numofbases) {
			...
			if (calculatePseudoEnergies) { 
				...
				SHAPEnew[position]   += CalculatePseudoEnergy(data, modifier, SHAPEslope, SHAPEintercept, true /*use_params_if_available*/ );
				SHAPEssnew[position] += CalculatePseudoEnergy(data, modifier, SHAPEslope_ss, SHAPEintercept_ss, false /*use_params_if_available*/ );
			} else { 
...
int structure::ReadExperimentalPairBonus(const char *filename, double const experimentalOffset, double const experimentalScaling ) {
	...
	if (!is_blank(filename)) {
	  ...
	  if ( count != numofbases * numofbases ){
		SetErrorDetails(sfmt("Found too few values in experimental bonus file '%s' -- found %i but expected %i.", filename, count, numofbases*numofbases));
...
const double structure::kT(5.904976983149999);
