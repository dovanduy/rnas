...
inline const char* nextNonWhitespaceChar(const char* input) { while(isspace(*input)) input++; return input; }
// Find the next non-whitespace character and return a pointer to it.
inline char* nextNonWhitespaceChar(char* input) { while(isspace(*input)) input++; return input; } // non-const version
